...
const PORT_FILE = process.env.MINDPOWERS_PORT_FILE || null;
...
function preferredPort() {
  if (process.env.MINDPOWERS_PORT) return Number(process.env.MINDPOWERS_PORT);
  if (PORT_FILE) {
    try {
...
const HOST = process.env.MINDPOWERS_HOST || '127.0.0.1';
const URL_HOST = process.env.MINDPOWERS_URL_HOST || (HOST === '127.0.0.1' ? 'localhost' : HOST);
const SESSION_DIR = process.env.MINDPOWERS_DIR || '/tmp/mindpowers-companion';
const CONTENT_DIR = path.join(SESSION_DIR, 'content');
const STATE_DIR = path.join(SESSION_DIR, 'state');
const MINDPOWERS_VERSION = readMindpowersVersion();
let ownerPid = process.env.MINDPOWERS_OWNER_PID ? Number(process.env.MINDPOWERS_OWNER_PID) : null;
...
// Persisted alongside the port (MINDPOWERS_TOKEN_FILE) so a restart keeps the
// same key and an already-open tab's cookie still validates.
const TOKEN_FILE = process.env.MINDPOWERS_TOKEN_FILE || null;
...
function initialToken() {
  if (process.env.MINDPOWERS_TOKEN) {
    return { value: process.env.MINDPOWERS_TOKEN, source: 'env' };
  }
  if (TOKEN_FILE) {
    try {
...
let COOKIE_NAME = 'mindpowers-key-' + PORT; // refined to the actual bound port in onListen
...
<html>
<head><meta charset="utf-8"><title>Mindpowers Companion</title>
<style>
...
<body><!-- BRANDING --><h1>Mindpowers Companion</h1>
...
function bootstrapPage(key) {
  ...
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Opening Mindpowers Companion</title></head>
<body>
<script>
try { sessionStorage.setItem('mindpowers-session-key', ${jsonKey}); } catch (e) {}
location.replace('/');
</script>
</body>
</html>`;
...
// browser is already connected. Override the launcher with MINDPOWERS_OPEN_CMD.
let browserOpened = false;
function maybeOpenBrowser() {
  if (browserOpened) return;
  browserOpened = true;
  if (!process.env.MINDPOWERS_OPEN) return; // opt-in: only after the user approves the companion
  if (HOST !== '127.0.0.1' && HOST !== 'localhost') return;
  if (clients.size > 0) return; // the user already opened it
  const url = companionUrl(); // must carry the key or the gate 403s it
  const cp = require('child_process');
  // Operator-provided launcher: run as given (this env var is trusted operator input).
  if (process.env.MINDPOWERS_OPEN_CMD) {
    try { cp.exec(process.env.MINDPOWERS_OPEN_CMD + ' ' + JSON.stringify(url), () => {}); } catch (e) { /* best effort */ }
    return;
  }
...
// override with MINDPOWERS_IDLE_TIMEOUT_MS (start-server.sh: --idle-timeout-minutes).
const IDLE_TIMEOUT_MS = (() => {
  const ms = Number(process.env.MINDPOWERS_IDLE_TIMEOUT_MS);
...
const LIFECYCLE_CHECK_MS = (() => {
  const ms = Number(process.env.MINDPOWERS_LIFECYCLE_CHECK_MS);
...
function startServer() {
  ...
  function onListen() {
    ...
    COOKIE_NAME = 'mindpowers-key-' + PORT;
  ...
  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE' && !triedFallback) {
      if (tokenSource === 'env') {
        console.error('Server failed to bind: preferred port is in use and MINDPOWERS_TOKEN is set; refusing fallback with explicit token');
        process.exit(1);
      }
      triedFallback = true;
      PORT = randomPort();
